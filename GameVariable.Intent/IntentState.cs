...
public partial struct IntentState : IIntent<IntentState.StateId, IntentState.EventId>
{
    public enum EventId : byte
    {
        ...
        CANCEL = 2,
        CANCELED_BEFORE_ACTIVATION = 3,
        CANCELED_BEFORE_RUN = 4,
        CHILD_TASK_CREATED = 5,
        COMPLETED_SUCCESSFULLY = 6,
        GET_READY = 7,
        RUN_AGAIN = 8,
        START_RUNNING = 9,
        UNABLE_TO_COMPLETE = 10,
    }
        ...
        switch (this.stateId)
        {
            ...
            case StateId.WAITING_FOR_ACTIVATION:
                switch (eventId)
                {
                    ...
                    case EventId.CANCELED_BEFORE_ACTIVATION: WAITING_FOR_ACTIVATION_canceled_before_activation(); break;
                }
                break;
            ...
            case StateId.WAITING_FOR_CHILDREN_TO_COMPLETE:
                switch (eventId)
                {
                    case EventId.ALL_CHILDREN_COMPLETED: WAITING_FOR_CHILDREN_TO_COMPLETE_all_children_completed(); break;
                }
                break;
            ...
            case StateId.WAITING_TO_RUN:
                switch (eventId)
                {
                    ...
                    case EventId.CANCELED_BEFORE_RUN: WAITING_TO_RUN_canceled_before_run(); break;
                }
                break;
        }
    ...
    private void WAITING_FOR_ACTIVATION_canceled_before_activation()
    {
        // WAITING_FOR_ACTIVATION behavior
        // uml: CANCELED_BEFORE_ACTIVATION TransitionTo(CANCELED)
        {
            // Step 1: Exit states until we reach `ROOT` state (Least Common Ancestor for transition).
            WAITING_FOR_ACTIVATION_exit();
    ...
    private void WAITING_TO_RUN_canceled_before_run()
    {
        // WAITING_TO_RUN behavior
        // uml: CANCELED_BEFORE_RUN TransitionTo(CANCELED)
        {
            // Step 1: Exit states until we reach `ROOT` state (Least Common Ancestor for transition).
            WAITING_TO_RUN_exit();
    ...
    public string EventIdToString(EventId id)
    {
        switch (id)
        {
            ...
            case EventId.CANCELED_BEFORE_ACTIVATION: return "CANCELED_BEFORE_ACTIVATION";
            case EventId.CANCELED_BEFORE_RUN: return "CANCELED_BEFORE_RUN";
