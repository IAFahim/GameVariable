...
namespace Variable.Inventory.Tests
{
    public partial class InventoryLogicTests
    {
        ...
        public void GetMaxAcceptable_Tests(float currentQty, float maxQty, float currentWeight, float maxWeight, float unitWeight, float expected)
        {
            Assert.Equal(expected, InventoryLogic.GetMaxAcceptable(currentQty, maxQty, currentWeight, maxWeight, unitWeight));
        }
        ...
        public void TryAddExact_Tests(float initial, float amount, float max, bool expectedResult, float expectedCurrent)
        {
            ...
            bool result = InventoryLogic.TryAddExact(ref current, amount, max);
        ...
        public void AddPartial_Tests(float initial, float amount, float max, float expectedAdded, float expectedOverflow, float expectedCurrent)
        {
            float current = initial;
            float added = InventoryLogic.AddPartial(ref current, amount, max, out float overflow);
        ...
        {
            float current = initial;
            bool result = InventoryLogic.TryRemoveExact(ref current, amount);
            Assert.Equal(expectedResult, result);
            Assert.Equal(expectedCurrent, current);
        }
        ...
        {
            float current = initial;
            float removed = InventoryLogic.RemovePartial(ref current, amount);
        ...
        public void TryTransferExact_Tests(float srcInitial, float dstInitial, float dstMax, float amount, bool expectedResult, float expectedSrc, float expectedDst)
        {
            float src = srcInitial;
            float dst = dstInitial;
            bool result = InventoryLogic.TryTransferExact(ref src, ref dst, dstMax, amount);
            Assert.Equal(expectedResult, result);
        ...
        [InlineData(10f, 0f, 90f, 100f, 10f, 10f, 2f, false, 10f, 0f, 90f)] // Weight limit exceeded (2 * 10 = 20, 90+20 > 100)
            ...
            bool expectedResult, float expectedSrc, float expectedDst, float expectedDstWeight)
        {
            float src = srcInitial;
            float dst = dstInitial;
            float dstWeight = dstWeightInitial;
            
            bool result = InventoryLogic.TryTransferExactWithWeight(
        ...
        public void TransferPartial_Tests(float srcInitial, float dstInitial, float dstMax, float amount, float expectedTransferred, float expectedSrc, float expectedDst)
        {
            float src = srcInitial;
            float dst = dstInitial;
            float transferred = InventoryLogic.TransferPartial(ref src, ref dst, dstMax, amount);
            ...
            float expectedTransferred, float expectedSrc, float expectedDst, float expectedDstWeight)
        {
            ...
            float transferred = InventoryLogic.TransferPartialWithWeight(
                ref src, ref dst, ref dstWeight,
                dstMaxWeight, dstMaxQty, unitWeight, amount);
