    ...
    IComparable
{
    ...
    /// </param>
    /// <param name="formatProvider">Optional format provider. Defaults to <see cref="CultureInfo.InvariantCulture" />.</param>
    /// <returns>Formatted string.</returns>
    public string ToString(string format, IFormatProvider formatProvider)
    {
        Span<char> buffer = stackalloc char[16];
        if (TryFormat(buffer, out var charsWritten, format, formatProvider ?? CultureInfo.InvariantCulture))
            return new string(buffer.Slice(0, charsWritten));
        ...
        IFormatProvider provider = null)
    {
        ...
        {
            var ratio = (double)this.GetRatio() * 100.0;
