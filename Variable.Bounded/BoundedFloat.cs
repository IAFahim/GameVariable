    ...
    IComparable
{
    ...
    {
        Span<char> buffer = stackalloc char[32];
        if (TryFormat(buffer, out var charsWritten, format.AsSpan(), formatProvider ?? CultureInfo.InvariantCulture))
        {
            return new string(buffer.Slice(0, charsWritten));
        }

        if (!string.IsNullOrEmpty(format) && format.ToUpperInvariant() == "R")
        {
            var ratio = this.GetRatio();
            return string.Format(formatProvider ?? CultureInfo.InvariantCulture, "{0:F1}%", ratio * 100.0);
        }
