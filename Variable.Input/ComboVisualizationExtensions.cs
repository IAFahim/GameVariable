...
public static class ComboVisualizationExtensions
{
        ...
        Func<int, string>? getInputName = null)
    {
        ...
        {
            var node = nodes[i];
            var isCurrent = i == state.CurrentNodeIndex;

            // Resolve Name
            var name = getActionName?.Invoke(node.ActionID) ?? $"Action {node.ActionID}";

            // Add Status if current
            if (isCurrent)
            {
                var status = state.IsActionBusy ? "BUSY" : "WAITING";
                name = $"{name}<br/>[{status}]";
            }
            ...
            if (node.EdgeCount > 0)
                for (var e = 0; e < node.EdgeCount; e++)
                {
                    var edgeIdx = node.EdgeStartIndex + e;
                    if (edgeIdx < edges.Length)
                    {
                        var edge = edges[edgeIdx];
                        var input = getInputName?.Invoke(edge.InputTrigger) ?? $"Input {edge.InputTrigger}";

                        // Highlight valid transitions from current node if not busy
                        var linkStyle = isCurrent && !state.IsActionBusy ? "==>" : "-->";
        ...
        var color = state.IsActionBusy ? "#ff4444" : "#44ff44"; // Red for busy, Green for waiting
